  ...
  const template = [
    ...
    // Window
    {
      ...
      submenu: [
        { role: "minimize" },
        ...(isMac
          ? [{ type: "separator" }, { role: "front" }, { role: "zoom" }]
          : [{ role: "close" }]),
      ],
    },
  ];
...
function registerIpcHandlers() {
  ...
  // ── Dialog ──
  ipcMain.handle("dialog:open", (e, opts) =>
    dialog.showOpenDialog(BrowserWindow.fromWebContents(e.sender), opts),
  );
  ipcMain.handle("dialog:save", (e, opts) =>
    dialog.showSaveDialog(BrowserWindow.fromWebContents(e.sender), opts),
  );
  ipcMain.handle("dialog:message", (e, opts) =>
    dialog.showMessageBox(BrowserWindow.fromWebContents(e.sender), opts),
  );

  // ── File system ──
  ipcMain.handle("fs:read-file", (_, filePath, opts) =>
    fs.promises.readFile(filePath, opts ?? "utf8"),
  );
  ...
  ipcMain.handle("fs:copy-file", (_, src, dest) =>
    fs.promises.copyFile(src, dest),
  );
  ipcMain.handle("fs:mkdir", (_, dirPath, opts) =>
    fs.promises.mkdir(dirPath, { recursive: true, ...opts }),
  );
  ipcMain.handle("fs:remove", (_, target) =>
    fs.promises.rm(target, { recursive: true, force: true }),
  );
  ipcMain.handle("fs:rename", (_, oldPath, newPath) =>
    fs.promises.rename(oldPath, newPath),
  );
  ipcMain.handle("fs:exists", (_, target) =>
    fs.promises
      .access(target)
      .then(() => true)
      .catch(() => false),
  ...
  ipcMain.handle(
    "os:version",
    () => process.getSystemVersion?.() ?? os.release(),
  );
  ...
  // ── Process ──
  ipcMain.handle("process:restart", () => {
    app.relaunch();
    app.exit(0);
  });
  ...
  ipcMain.handle("window:unmaximize", (e) =>
  ...
  ipcMain.handle("window:set-fullscreen", (e, flag) =>
  ...
  ipcMain.handle("global-shortcut:unregister", (_, accelerator) =>
    globalShortcut.unregister(accelerator),
  );
  ipcMain.handle("global-shortcut:is-registered", (_, accelerator) =>
